...
fetch(
  'https://opendata.paris.fr/api/records/1.0/search/?dataset=que-faire-a-paris-&q=&rows=300&facet=date_start&facet=date_end&facet=tags&facet=address_name&facet=address_zipcode&facet=address_city&facet=pmr&facet=blind&facet=deaf&facet=transport&facet=price_type&facet=access_type&facet=updated_at&facet=programs&refine.tags=Expo&refine.date_start=2023&refine.date_end=2024'
)
  .then((response) => response.json())
  .then((response2) => console.log(response2.records));

fetch(
  'https://opendata.paris.fr/api/records/1.0/search/?dataset=que-faire-a-paris-&q=&rows=1000&facet=date_start&facet=date_end&facet=tags&facet=address_name&facet=address_zipcode&facet=address_city&facet=pmr&facet=blind&facet=deaf&facet=transport&facet=price_type&facet=access_type&facet=updated_at&facet=programs&refine.tags=Expo&refine.date_start=2023&refine.date_end=2024'
)
  ...
  .then((data) => {
    for (let i = 0; i < data.records.length; i++) {
      ...
      const eventAddress_street = data.records[
        i
      ].fields.address_street.replaceAll(',', '');
      ...
      let eventAddress =
        eventAddress_street +
        ', ' +
        eventAddress_zipcode +
        ' ' +
        eventAddress_city;
      const eventDate = data.records[i].fields.date_description.replaceAll(
        '/[<]br[^>]*[>]/gi',
        ''
      );
